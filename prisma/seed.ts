import { PrismaClient, Role, TeamType, MatchStatus, SlotStatus, JoiningStatus } from '@prisma/client';
...
async function main() {
  ...
  const today2130 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 21, 30, 0);
  ...
  for (let s = 1; s <= 48; s++) {
    ...
    await prisma.slot.upsert({
      ...
      create: {
        ...
        teamNumber: teamNum,
        position: pos,
        status: s === 1 ? SlotStatus.OCCUPIED : SlotStatus.AVAILABLE,
  ...
  await prisma.notification.create({
    data: {
      userId: player1.id,
      title: 'Welcome to VOID X ARENA',
      message: 'Your contender account is verified. Register for daily Free Fire custom rooms and compete for rewards.',
      type: 'ANNOUNCEMENT',
    },
  });
